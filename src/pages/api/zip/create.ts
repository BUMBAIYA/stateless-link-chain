...
import { validateZipBoard } from "@/lib/zip/validate";

/**
 * Create a new zip game chain. Board: -1=blocked, 0=empty path, 1..waypointCount=waypoints.
 * Body: { gridSize: 4|5|7, board: number[], waypointCount: number }
 */
export const POST: APIRoute = async ({ request }): Promise<Response> => {
  let body: { gridSize?: number; board?: number[]; waypointCount?: number };
  try {
  ...
  const { gridSize, board, waypointCount } = body;
